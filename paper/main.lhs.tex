...
We have implemented this optimization, and have evaluated it on some formulas, with promising results. However, we have not had enough time to properly evaluate this optimization for this version of the paper, but we are confident we can do this before we prepare the final version of this paper.
...
~\\ \noindent{\bf Results}$\;$ Surprisingly, the method in this paper can solve most problems within 1 second, and {\bf almost all problems within 5 seconds}. The only two problems that do not fall into this category are:
...
These results may seem impressive, but as a note to the reader, the set of available standard benchmarks is way too small to draw any significant conclusion! Also, the prover in this paper was developed without access to any of these benchmarks on beforehand.
